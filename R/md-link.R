...
#' @description Take a vector of link texts and link destinations and return
#'   single vector of valid markdown inline links. These inline links can then
#'   be rendered as `<href>` HTML tags.
#' @details
...
#' @description Take a vector of image alt texts and image link destinations and
#'   return single vector of valid markdown image links. These inline image
#'   links can call images to be rendered as `<img>` HTML tags.
#' @details Syntax for images is like the syntax for links, with one difference.
#'   Instead of link text, we have an image description. The rules for this are
#'   the same as for link text, except that (a) an image description starts with
#'   `![` rather than `[`, and (b) an image description may contain links. An
#'   image description has inline elements as its contents. When an image is
