...
#'   same type. Two list markers are of the same type if (a) they are bullet
#'   list markers using the same character (`-`, `+`, or `*`)...
...
#'
#'   Two list items are of the same type if they begin with a list marker of the
#'   same type. Two list markers are of the same type if (b) they are ordered
#'   list numbers with the same delimiter (either `.` or `)`).
#'
...
#' @details GFM enables the tasklist extension, where an additional processing
...
#'   checkbox element; in an HTML output, this would be an `<input
...
#'   inmutable elements, or they may dynamically handle dynamic interactions
#'   (i.e. checking, unchecking) in the final rendered document.
