...
#' @details A block quote marker consists of 0-3 spaces of initial indent, plus
#'   (a) the character `>` together with a following space, or (b) a single
#'   character `>` not followed by a space.
#'
#'   The following rules define block quotes:
#'   1. **Basic case.** If a string of lines _Ls_ constitute a sequence of
#'   blocks _Bs_, then the result of prepending a block quote marker to the
#'   beginning of each line in _Ls_ is a block quote containing _Bs_.
...
#'   block quote marker is paragraph continuation text is a block quote with
#'   _Bs_ as its content. Paragraph continuation text is text that will be
