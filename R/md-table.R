...
#' @details From the [GFM spec](https://github.github.com/gfm/#table):
#'   A table is an arrangement of data with rows and columns, consisting of a
#'   single header row, a
#'   [delimiter row](https://github.github.com/gfm/#delimiter-row) separating
#'   the header from the data, and zero or more data rows... Each row consists
#'   of cells containing arbitrary text, in which
...
#'   reading, and if there’s otherwise parsing ambiguity. Spaces between pipes
#'   and cell content are trimmed. Block-level elements cannot be inserted in a
#'   table. The delimiter row consists of cells whose only content are hyphens
