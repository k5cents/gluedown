...
The `gluedown` package uses [`glue()`][glue] to wrap character vectors in
markdown syntax. Markdown is a lightweight, plain-text markup language prevalent
in the R community.
...
Creating numbered and bullet lists from vectors was the initial inspiration for
the package. Here, we also see how _inline_ functions can be used within _block_
functions.
...
The package primarily uses [GitHub Flavored Markdown][gfm] (GFM), a a
site-specific version of the [CommonMark specification][cm], an unambiguous
implimentation of the John Gruber's [original Markdown][df]. With this spec,
features like some useful extensions [GitHub task lists][task] are supported.
