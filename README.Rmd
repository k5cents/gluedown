...
Some features aren't technically supported by GFM, but can be forced by pandoc. With `md_define()`,
definition lists are simply rendered as unordered lists.

```{r ex_define, results='asis'}
md_define("Democracy", "Government by the people", cat = TRUE)
```

The `md_fence()` function makes code blocks, which is kind of superfluous in R Markdown, but can
still be useful with `base::deparse()` or `readr::read_lines()`.
...
The `md_table()` function wraps around the fantastic [`knitr::kable()`][kable] function to create
