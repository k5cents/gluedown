...
> This means that parsing can proceed in two steps: first, the block structure
of the document can be discerned; second, text lines inside paragraphs,
headings, and other block constructs can be parsed for inline structure. The
second step requires information about link reference definitions that will be
available only at the end of the first step. Note that the first step requires
processing lines in sequence, but the second can be parallelized, since the
inline parsing of one block element does not affect the inline parsing of any
other.
...
regular markdown text. This can be useful with R functions like those from
the `rvest` package, which reads web pages.
...
Paragraphs are one are where it's easy for markdown newcomers can have 
difficulty writing the plain text in the way they need to result in the text
they want. Words separated by a single newline are rendered as a single
paragraph `<p>` tag. Using `md_paragraph()`, we can take a vector of character
text and output _distinct_ paragraphs.
