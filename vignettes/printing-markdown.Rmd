...
takes a plaint text special `.Rmd` file containing code "chunks" and executes
that code when converting to some other format (`.md`, `.html`, `.pdf`, etc).
...
This is more readable, but really just prints as a couple lines of the document.
With some more work, we can use `cat()` to print an ordered list.

```{r cat_order, results='asis'}
cat(paste0(1:3, ". ", state.name[1:3]), sep = "\n")
```

This workflow get's tiresome, although it's made slightly more simple with
...
Aside from container blocks and inlines, there is a third type of markown
...
Code blocks are another type of leaf block. The code I've been writing so far
...
other types of text. Prhaps we want to show the content of a function. The
...
md_fence(paste("$", command), char = "~", lang = "bash")
```

Or maybe we simply want to display some preformated text that only works in a
monospace font like that which is used in code blocks.

```{r indent_block, results='asis'}
cat <- c(
  " -------------- ", "Gluedown rules!" ," --------------",
  "    \\", "     \\", "      \\", "         /\\_/\\",
  "        ( o.o )", "         > ^ <"
...
Markdown tables are another extremelly useful extension. The `md_table()`
